(ns ninjavan.main
  "Kotodama WASM entrypoint for the Ninjavan clean-room actor (L4) — Clojure port.
  ...
  facts (`ninjavan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ninjavan")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "ninjavan_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "ninjavan_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"ninjavan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ninjavan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ninjavan-compat/ready)
